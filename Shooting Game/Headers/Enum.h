#pragma once
#ifndef __ENUM_H__
#define __ENUM_H__
namespace OBJ//오브젝트 종류
{
	enum ID {BACKGROUND,BUTTON,BULLET,BOSSBULLET,MONBULLET, SCREWBULLET, HOMINGBULLET, MONSTER,BOSS, PLAYER,SHIELD, MOUSE, HPBAR,END};
	enum BACK {CLOUD, BIRD, SUN,HUMAN,SHIP,BACK_END};
}
namespace BUTTON//버튼 종류
{
	enum NAME{START,QUIT,REMOVE,SAVE,END};
}
namespace SCENE//씬 종류
{
	enum TAG{START,STAGE1,END};
}
namespace EVENT//이벤트 종류
{
	enum TRIGGER{DEAD, GAME_START, GAME_END, NOEVENT, END};
}
namespace SCENE_STATE//씬 상태
{
	enum STATE{NO,CHANGE,END};
}
namespace BOSS
{
	enum STAGE {STAGE1,STAGE_END};
	enum PATTERN {BASIC=0,PATTERN_1, PATTERN_2, PATTERN_3, PATTENR_END};
}

namespace ENEMY
{
	enum TYPE { ENEMY_1, ENEMY_2, ENEMY_3, TYPE_END };
	enum PATTERN { PATTERN_1, PATTERN_2 , PATTERN_3, PATTERN_END };
	enum DIR { TOP, LEFT, RIGHT, BOTTOM, DIR_END };
}
namespace EDIT_OBJ
{
	enum ID {MOUSE,BUTTON,END};
}
namespace EDIT_EVENT
{
	enum EVENT{NODRAW,REMOVE,SAVE,NOEVENT};
}
namespace DIRECTION
{
	enum DIR{LEFT, RIGHT, UP, DOWN, END};
}
#endif